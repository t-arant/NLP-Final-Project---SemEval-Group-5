...
def generate_BIOES_labels(lang, task, subtask, domain, tkr_name, scheme, file_name, max_len):
    
    tokenizer = AutoTokenizer.from_pretrained(tkr_name)
...
def test_labeling(df, tknzr, lang, domain, max_tok, tok_type):
    ...
    with open(f"incorrect_reconstructions_{lang}_{domain}_{tok_type}_sep.txt", "w") as f:
    ...
    df.to_csv(f"{lang}_{domain}_{tok_type}_sep.csv")

    with open(f"{lang}_{domain}_{tok_type}_drops_sep.txt", "w") as f2:
        f2.write("[")
