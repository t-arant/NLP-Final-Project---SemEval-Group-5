...
# DATASET = "/home/tony-arant/NLP Final Project/st2_crf/subtask_2_eng_laptop_opn_asp_split_monolingual2.json"
...
class TransformerTagger(nn.Module):
    ...
    def train_epoch_tagger(self, dataloader, optimizer, device, type_flag):
        self.train()
        total_loss = 0
    ...
    def eval_epoch(self, dataloader, device, flag):
        self.eval()
        total_loss = 0
        with torch.no_grad():
...
def generate_final_predictions(reg_mod_path, asp_mod_path, opn_mod_path, mod_n, dataloader, device, dataframe, tokenizer, lang):
    # model.eval()
    ...
    reg_model = TransformerRegressor(mod_n).to(device)
    ...
    # Preparing Aspect Extractor
    asp_tag_model = TransformerTagger(mod_n).to(device)
    ...
    for batch in dataloader:
        ...
        for i in range(len(pred_labs)):
            ...
            if asp_emb:
                ...
                if lang == "rus":
                    asp = re.sub(r' - ', '-', asp)
                    asp   = re.sub(r'([0-9]) \+', r'\1+', asp)
                    asp   = re.sub(r'([0-9]) %',  r'\1%', asp)
                    asp = re.sub(r'"\s*([^"]*?)\s*"', r'"\1"', asp)
    ...
    opn_tag_model = TransformerTagger(mod_n).to(device)
    ...
    for batch in dataloader:
        ...
        for i in range(len(pred_labs)):
            ...
            if opn_emb:
                ...
                if lang == "rus":
                    opn = re.sub(r' - ', '-', opn)
                    opn  = re.sub(r'([0-9]) \+', r'\1+', opn)
                    opn  = re.sub(r'([0-9]) %',  r'\1%', opn)
                    opn = re.sub(r'"\s*([^"]*?)\s*"', r'"\1"', opn)
