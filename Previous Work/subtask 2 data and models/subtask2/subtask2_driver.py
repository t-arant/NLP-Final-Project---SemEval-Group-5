...
def main():
    ...
    if train == 1:
        ...
        train_dataset = VA_ASP_OPN_Dataset(train_df, tokenizer)
        train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)

        dev_dataset = VA_ASP_OPN_Dataset(dev_df, tokenizer)
        ...
        model = BERT_Tag_Reg_Model().to(device)
        ...
        for epoch in range(t_epochs):
            train_loss = train_tagger_epoch(model, train_loader, optimizer, device)
        
            # for validation loss, we can create a similar eval function
            val_loss = eval_tagger_epoch(model, dev_loader, device)
        ...
        print("\nTrain Loss:")
        final_train_loss_r, final_train_loss_t = model_inference(model, va_loss_fn, train_loader_whole, device)
        print(f"Reg: {final_train_loss_r}\tTag: {final_train_loss_t}")
        print("Val Loss:")
        final_val_loss_r, final_val_loss_t = model_inference(model, va_loss_fn, dev_loader_whole, device)
    ...
    # Returns a JSONL of VA predictions from text
        ...
        model = BERT_Tag_Reg_Model()
        model.load_state_dict(torch.load("english_laptop_final_subtask_2.pth"))
        model = model.to(device)

        # Tokenize test datasets, and load into DataLoader
        test_dataset = VA_ASP_OPN_Dataset(test_df, tokenizer)
        test_loader = DataLoader(test_dataset, batch_size=64, shuffle=False)

        output = generate_results(model, test_loader, device, test_df, tokenizer)
        
        # write predictions to orgainizer specified formate file
        with open("subtask_2_eng_laptop_preds.json", "w", encoding="utf-8") as f:
            for out in output:
               f.write(json.dumps(out, ensure_ascii=False) + "\n")

        # generate_gold_file(test_df, "subtask_2_eng_laptop_golds.json")
