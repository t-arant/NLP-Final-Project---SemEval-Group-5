...
def main():
    ...
    if train == 1:
        ...
        # Include architecture info in filename for better tracking
        arch_info = f"hd_{'_'.join(map(str, hidden_dims))}_act_{activation}"
        loss_df = pd.DataFrame(train_losses, columns=['Epoch', 'Train Loss', 'Validation Loss'])
        loss_df.to_csv(f"loss_graphs/subtask1_{trial}_{arch_info}.csv", index=False)
